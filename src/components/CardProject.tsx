...
const CardProject = ({ Img, Title, Description, Link: ProjectLink, id }) => {
  ...
  return (
    <>
            ...
            {ProjectLink && (
              <>
                <button
                  ...
                  className="p-2 rounded-lg bg-[#6366f1]/80 backdrop-blur-md border border-white/10 text-white hover:bg-[#6366f1] transition-all duration-200 shadow-lg"
                  title="Simulated Live Frame Preview"
                >
          ...
          <div className="flex items-center justify-between pt-2 border-t border-white/[0.05]">
            {ProjectLink ? (
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => setIsPreviewOpen(true)}
                  className="text-[11px] text-white/70 hover:text-emerald-400 flex items-center gap-1 transition-colors"
                >
                  <Monitor className="w-3 h-3 text-emerald-400" /> Frame View
                </button>
              </div>
            ) : (
      ...
      {/* Live Preview Simulated Frame Modal */}
      {ProjectLink && (
        <LivePreviewModal
          isOpen={isPreviewOpen}
          onClose={() => setIsPreviewOpen(false)}
          projectUrl={ProjectLink}
          projectTitle={Title || "Project Live Preview"}
        />
      )}
    </>
  );
